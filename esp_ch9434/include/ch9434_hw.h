/*
 * CH9434 硬件访问层（SPI 协议 + 寄存器操作）。
 *
 * CH9434 需要 SPI 模式 0（CPOL=0, CPHA=0）主机。每次 CS 低事务
 * 携带一个命令字节和一个数据字节，两者之间有 >1us 的间隔。
 * 本模块使用 ESP-IDF SPI 主机驱动实现该时序，并通过专用
 * FreeRTOS 服务任务将所有 SPI 访问串行化，保证线程安全。
 *
...
#ifndef CH9434_HW_SPI_CLOCK_HZ
#ifdef CONFIG_CH9434_SPI_CLOCK_HZ
#define CH9434_HW_SPI_CLOCK_HZ CONFIG_CH9434_SPI_CLOCK_HZ
#else
#define CH9434_HW_SPI_CLOCK_HZ (2 * 1000 * 1000)
#endif
#endif

/**
 * 初始化 CH9434 硬件接口（SPI 总线 + 服务任务）。
 *
 ...
 *
 * @param reg        8 位寄存器地址（不含操作位）。
 * @param clear_mask 要清零的位掩码。
 * @param set_mask   要置位的位掩码。
 * @return 成功返回 ESP_OK，否则返回 esp_err_t 错误码。
 */
